...
const components = { About, SnakeGame, Contact, Terminal, Github: null };
let nextWindowId = 0;
...
function createWindowManager() {
    ...
    return {
        subscribe,
        open: (app) => {
            // Special handling for Notes - redirect to blog
            if (app.id === 'Notes') {
            ...
            update(state => {
                ...
                if (isMobile) {
                    ...
                    if (app.id === 'SnakeGame') {
                        position = { x: Math.max(10, (screenWidth - 1000) / 2), y: Math.max(40, (screenHeight - 800) / 2) };
                    } else if (app.id === 'SnakeGame') {
                        position = { x: Math.max(10, (screenWidth - 600) / 2), y: Math.max(40, (screenHeight - 700) / 2) };
                    } else if (app.id === 'Terminal') {
